...
class Auth extends BaseController
{
                ...
                if (password_verify($password, $user['password'])) {
                    if ($user['is_active'] == 1) {
                        ...
                        if ($user_session['user_level'] == 1) {
                            return redirect()->to('/admin/pengaduan/di-proses');
                        } else if($user_session['user_level'] == 2) {
                            return redirect()->to('/admin/pengaduan/masuk');
                        } else if($user_session['user_level'] == 4){
    ...
    {  $selectedLokasi = $this->request->getPost('lokasi');
        $selectedLevel = $this->request->getPost('user_level');
        if (!isset($_POST['btn-submit'])) {
            return redirect()->to('/notfound');
        }
        ...
        $rules = [
            ...
            'user_level' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'Jenis Pegguna wajib dipilih.'
                ]
            ],
            'lokasi' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'Lokasi RT wajib dipilih.'
                ]
            ],
            'password' => [
            ...
            'user_ktp' => [
                ...
                'errors' => [
                    ...
                    'max_size' => 'Ukuran file ktp tidak boleh lebih dari 512 Kb.',
        ...
        if (!$this->validate($rules)) {
            ...
            session()->setFlashdata('msg-auth', 'Pendaftaran berhasil. Silahkan menunggu <b>1 x 24 jam</b>, untuk aktivasi akun oleh petugas.');
            return redirect()->to('/auth');
        }
    }
